...
const Navbar = ({ scrollOffset }: { scrollOffset: number }) => {
	...
	return (
		<motion.header
			className={`fixed duration-500 max-w-[100vw] inset-x-0 top-0 z-50  ${
				scrollOffset >= 20
					? 'bg-white shadow-lg'
					: 'bg-transparent border-b  border-b-slate-300 lg:bg-slate-700'
			} `}
			initial={{ opacity: 0 }}
		...
		>
			<div className='container w-full '>
				<nav
					className='flex items-center flex-col justify-center  py-4 w-full'
					aria-label='Global'
				>
          ...
          <a href="#" className="-m-1.5 p-1.5">
            <h2 className='font-bold text-lg text-slate-700'>Logo</h2>
					...
					<div className='flex lg:hidden w-full'>
						<button
							type='button'
							className='-m-2.5 inline-flex items-center justify-center rounded-md p-2.5 text-gray-700 ms-auto'
						...
						{navigation.map((item) => (
							<a
								...
								className={`rounded-lg px-3 py-1 text-xs font-bold tracking-wide ${
									scrollOffset < 20 ? 'text-slate-100' : 'text-slate-600'
								} leading-6  hover:bg-blue-50 duration-300 hover:text-slate-600`}
							>
								{item.name}
							</a>
						))}
